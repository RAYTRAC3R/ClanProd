...
class Button(object):
    worldmap = pygame.Surface((1280,1280))
    used_screen = worldmap  # The surface to draw the text on
    ...
    def draw_maptile_button(self, pos, available=True, image=None, **values):
        is_clickable = False
        button = pygame.transform.scale(image, (192, 35))
        collided = self.used_screen.blit(button, pos)
        if available and collided.collidepoint(self.used_mouse.pos):
